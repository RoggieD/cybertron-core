...
def render_docker_health_summary(result: dict) -> str:
    ...
    if unhealthy:
        ...
        lines.append("No running container explicitly reports Docker health status 'unhealthy'.")
    ...
    if stopped:
        lines.extend(["", "Stopped/exited (not classified as Docker-health unhealthy):"])
        for item in stopped:
            lines.append(f"- {item.get('name')} | {item.get('status')}")
